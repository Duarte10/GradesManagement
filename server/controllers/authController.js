    ...
    if (error) {
      ...
      if (user) {
        ...
        if (passwordMatch) {
          ...
          expiresAt.setTime(expiresAt.getTime() + config.jwtExpires * 1000)
          return res.status(httpStatus.OK).send({ auth: true, token: signedToken, expiresAt: expiresAt.getTime() });
        } else {
